                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBCL ought to be enough for anybody." - Bill Gates, 2017
    ui->zBCLpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBCLSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBCL->setEnabled(false);
        ui->pushButtonMintzBCL->setToolTip(tr("zBCL is currently disabled due to maintenance."));

        ui->pushButtonSpendzBCL->setEnabled(false);
        ui->pushButtonSpendzBCL->setToolTip(tr("zBCL is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBCLpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBCL_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBCL is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBCL...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBCL in ") +
...
void PrivacyDialog::on_pushButtonSpendzBCL_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBCL is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBCL
        sendzBCL();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBCL
    sendzBCL();
}

void PrivacyDialog::on_pushButtonZBclControl_clicked()
{
    ZBclControlDialog* zBclControl = new ZBclControlDialog(this);
    zBclControl->setModel(walletModel);
    zBclControl->exec();
}

void PrivacyDialog::setZBclControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBclSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBCL()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Bitcoinlegend Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBCLpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBCL is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BCL </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBCL</b>";
    ...
    // use mints from zBcl selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBclControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBclControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbcl selector in case it was used
    ZBclControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBcl Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBCL </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBCL "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBCL "));
    ui->labelzBCLAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBCLSyncStatus->setVisible(fShow);
}
