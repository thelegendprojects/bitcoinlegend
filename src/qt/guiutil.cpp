...
#define URI_SCHEME "bitcoinlegend"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bitcoinlegend address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no Bitcoinlegend: URI
...
{
    // Convert bitcoinlegend:// to bitcoinlegend:
    //
    //    Cannot handle this later, because bitcoinlegend:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open bitcoinlegend.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitcoinlegend.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Bitcoinlegend.lnk
...
{
    return GetAutostartDir() / "bitcoinlegend.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a bitcoinlegend.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Bitcoinlegend\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bitcoinlegend app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add bitcoinlegend app to startup item list
