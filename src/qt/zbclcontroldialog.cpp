...
#include "ui_zbclcontroldialog.h"
...
std::list<std::string> ZBclControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBclControlDialog::listMints;

ZBclControlDialog::ZBclControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBclControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBclControlDialog::~ZBclControlDialog()
{
    delete ui;
}

void ZBclControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBclControlDialog::updateList()
{
...
void ZBclControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBclControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBcl_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBclControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBclControlDialog::GetSelectedMints()
{
...
void ZBclControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
